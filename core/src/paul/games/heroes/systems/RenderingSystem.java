...
public class RenderingSystem extends SortedIteratingSystem {
    ...
    static final float TILE_SIZE = 16.0f;
    static final float PPM = 1.0f; // pixels per meter
    static final float FRUSTUM_WIDTH = Gdx.graphics.getWidth()/PPM;
    static final float FRUSTUM_HEIGHT = Gdx.graphics.getHeight()/PPM;
    static final float PIXELS_TO_METRES = 1.0f/PPM;
    ...
    private Comparator<Entity> comparator; // comparator that allows us to sort images based on z position
    ...
    public void update(float deltaTime) {
        ...
        for (Entity entity : renderQueue) {
            ...
            batch.draw(tex.region,
                    t.position.x * TILE_SIZE, t.position.y * TILE_SIZE,
                    originX, originY,
                    width, height,
    ...
    private static class ZComparator implements Comparator<Entity> {
        ...
        public int compare(Entity e1, Entity e2) {
            return (int)Math.signum(tcMapper.get(e1).position.z - tcMapper.get(e2).position.z);
        }
    }
}
